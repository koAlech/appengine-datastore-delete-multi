...
func handleCron(w http.ResponseWriter, r *http.Request) {
	...
	case "daily_delete_old_act_logs":
		...
		//--------------------------
		log.Infof(c, "[CRON] Deleting all old records!<hr>")
		...
		for recCount > 0 && max_reached == false {
			...
			log.Infof(c, "LOOP [%v] -=> saw [%v] old records<br>", attempt, recCount)
			...
			for i := 0; i < len(keys); i += REC_PER_DEL_JOB {
				...
				goRoutine++
				//Delete each chunk
				log.Infof(c, "[GO-ROUTINE]# %v<br>", goRoutine)
				...
				if goRoutine > MAX_JOBS_TO_BE_RUN {
					...
					log.Infof(c, "MAX_JOBS_TO_BE_RUN!")
					max_reached = true
					break
				}
		...
		if attempt >= 1 {
			log.Infof(c, "<hr>LOOP TOTAL: %v<br>", attempt)
		}
...
func deleteBatchMulti(c context.Context, delChan chan bool, keys []*datastore.Key) {
